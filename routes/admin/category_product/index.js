...
module.exports = function(router) {
    router.get('/category_product/grid', authUser.checkTokenAdmin, (req, res) => {
        try {
            var obj = {
    ...
    router.post('/category_product/form', authUser.checkTokenAdmin ,(req, res) => {
        try {
     ...
     * params: code: String
     */
    router.get('/category_product/code', authUser.checkTokenAdmin, (req, res) => {
        try {
    ...
    router.get('/category_product', authUser.checkTokenAdmin, (req, res) => {
        var sql = `CALL ${store.category_product.getAll}()`
        mysql.lib.query(sql, [true] , function(err, results, fields) {
            ...
            else res.status(500).json({status: 500, message: 'category_product not found', data: [] })
        })
    })

    router.get('/get-news-by-category_product', authUser.checkTokenAdmin, (req, res) => {
        var {key, pageSize, pageNumber, category_product} = req.query
        var offset = utility.offset(pageSize, pageNumber)
        var whereCategoryNews = ''
        if (category_product) {
            whereCategoryNews = `and news_category_product_id='${category_product}'`
        }
    ...
    router.get('/category_product/update', authUser.checkTokenAdmin, (req, res) => {
        try {
            var { data } = req.query
    ...
    router.get('/category_product/delete', authUser.checkTokenAdmin, (req, res) => {
        try {
            var { code } = req.query
